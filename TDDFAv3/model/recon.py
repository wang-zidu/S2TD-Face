...
class face_model:
    def __init__(self, args):
        ...
        if args.backbone == 'resnet50':
            ...
            self.net_recon.load_state_dict(torch.load("./TDDFAv3/assets/net_recon.pth", map_location=torch.device('cpu'))['net_recon'])
        ...
        if args.backbone == 'mbnetv3':
            ...
            self.net_recon.load_state_dict(torch.load("./TDDFAv3/assets/net_recon_mbnet.pth", map_location=torch.device('cpu'))['net_recon'])
            self.net_recon = self.net_recon.to(self.device)
            self.net_recon.eval()
    ...
    def forward_uv(self, no_pca = False):
        ...
        _, _, _, visible_idx_renderer = self.renderer(v3d.clone(), self.tri, torch.clamp(face_texture, 0, 1).clone(), visible_vertice = True)
